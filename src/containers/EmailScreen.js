		...
		<div className="layout">
			...
			<div className={Style.container}>
				<div className={Style.row}>
					<div className={Style.coverMessage}>
						<div className={Style.message}>
							Un devis vous sera envoyé par mail avec un récapitulatif de votre demande.
						</div>
					</div>
					...
					<div className={Style.fixer}>
						...
						{error && !data.emailAddress ? (
							<div className={Style.space}>
								{' '}
								<ErrorMsg error={error} text="Veuillez saisir votre adresse email" />
							</div>
						) : null}
					</div>
				</div>
				<div className={Style.rowToCheck}>
					...
					/>
					<div onClick={() => handleState({ isChecked: !data.isChecked })}>
						J'accepte de recevoir par email des propositions de MeilleurTaux.com
					</div>
				</div>
				{error && !data.isChecked ? (
					<div className={Style.bigSpace}>
						{' '}
						<ErrorMsg error={error} text="Veuillez accepter la récéption d'email" />
					</div>
				) : null}
			</div>
		</div>
