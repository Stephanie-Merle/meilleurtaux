...
function reducer(data, action) {
	//using reducer to update simultaneously state and cookie
	switch (action.type) {
		case 'SET_DATA': {
			// equivalent of setState({...data, ...newData})
		...
		case 'RESET_DATA': {
			//removing cookie when information is stored and email sent
...
const App = () => {
	...
	const handleError = (page) => {
		// return errors when some required information are missing
		if (page === 4) {
			if (!data.zip) {
				...
				<Switch>
					...
					<Route path="/Confirmation">
						...
						<Navbar
							...
							next={data.isChecked && data.emailAddress ? '/LastScreen' : '/Confirmation'}
						/>
					</Route>
					<Route path="/Quote">
						<Quote
						...
						/>
						<Navbar
							prev="/PropertyLocation"
							page={5}
							handleError={() => handleError(5)}
							next={data.landCost && data.estimatedPrice ? '/Confirmation' : '/Quote'}
						/>
					</Route>
					<Route path="/PropertyLocation">
						<PropertyLocation
							handleLocation={(el) =>
								dispatch({
									type: 'SET_DATA',
									newData: { country: el.country, zip: `${el.city} (${el.code})` }
								})}
							error={errorLocation}
						...
						<Navbar
							...
							next={data.zip ? '/Quote' : '/PropertyLocation'}
						/>
					</Route>
					{display.reverse()}
					<Route path="/">
